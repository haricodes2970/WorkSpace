...
import { ScopePanel } from "@/features/execution/scope/scope-panel";
import { ReviewList } from "@/features/execution/reviews/review-list";
...
import type { ScopeItemData } from "@/features/execution/scope/scope-panel";
import type { ReviewData } from "@/features/execution/reviews/review-list";
import type { ReviewFormData } from "@/features/execution/reviews/review-form";
import type { DecisionData, DecisionFormInput } from "@/features/execution/decisions/decision-log";
import type { TimelineEventData } from "@/features/execution/timeline/timeline-view";
...
interface ProjectCommandCenterProps {
  ...
  onAddTask: () => void;
  onShip: () => void;
  onSaveReview: (data: ReviewFormData) => Promise<void>;
  onAddDecision: (data: DecisionFormInput) => Promise<void>;
  onReverseDecision: (id: string, note: string) => Promise<void>;
  onDeleteDecision: (id: string) => Promise<void>;
  onAddScopeItem: (title: string, bucket: ScopeBucket) => Promise<void>;
...
export function ProjectCommandCenter({
  ...
  // Calculate live momentum on client
        ...
        <div className="px-6 py-5 border-b border-[--color-border-subtle] bg-[--color-panel] shrink-0">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2.5 mb-1.5 flex-wrap">
                ...
                <MomentumBadge state={momentum.state} score={momentum.score} />
          ...
          <div className="flex items-center gap-4 mt-3 flex-wrap">
            ...
            {activeBlockers.length > 0 && (
              <span className="text-[12px] text-[--color-error] flex items-center gap-1">
          ...
          <TabsList className="px-6 bg-[--color-panel] shrink-0">
            <TabsTrigger value="tasks">
              ...
              {project.tasks.length > 0 && (
                <span className="ml-1.5 text-[10px] text-[--color-text-muted]">{project.tasks.length}</span>
              )}
            </TabsTrigger>
            <TabsTrigger value="milestones">
              Milestones
              {project.milestones.length > 0 && (
                <span className="ml-1.5 text-[10px] text-[--color-text-muted]">{project.milestones.length}</span>
              )}
            </TabsTrigger>
            <TabsTrigger value="reviews">
              Reviews
              {project.reviews.length > 0 && (
                <span className="ml-1.5 text-[10px] text-[--color-text-muted]">{project.reviews.length}</span>
              )}
            </TabsTrigger>
            <TabsTrigger value="decisions">
              Decisions
              {project.decisions.length > 0 && (
                <span className="ml-1.5 text-[10px] text-[--color-text-muted]">{project.decisions.length}</span>
              )}
            </TabsTrigger>
            ...
            <ScrollArea className="h-full">
              <div className="px-6 py-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button size="sm" variant="secondary" onClick={onAddTask} className="gap-1.5 text-[12px] h-7">
                    <Plus className="h-3.5 w-3.5" /> Add task
                  </Button>
                </div>
                {project.tasks.length === 0 ? (
                  <EmptyState icon={<CheckSquare className="h-8 w-8" />} message="No tasks yet" action={{ label: "Add first task", onClick: onAddTask }} />
                ) : (
            ...
            <ScrollArea className="h-full">
              <div className="px-6 py-4 flex flex-col gap-3">
                {project.milestones.length === 0 ? (
                  ...
                  project.milestones.map((m, i) => <MilestoneCard key={m.id} milestone={m} index={i} />)
                )}
              </div>
            </ScrollArea>
          </TabsContent>
                ...
                {project.notes.length === 0 ? (
                  ...
                  project.notes.map((note) => (
                    <div key={note.id} className="rounded-lg border border-[--color-border] bg-[--color-card] p-4">
                      {note.title && (
                        <p className="text-[13px] font-medium text-[--color-text-primary] mb-1.5">{note.title}</p>
                      )}
                      <p className="text-[13px] text-[--color-text-secondary] whitespace-pre-wrap leading-relaxed">
                        {note.content}
                      </p>
                      <p className="text-[11px] text-[--color-text-muted] mt-3">{formatRelativeTime(note.updatedAt)}</p>
                    </div>
                  ))
                )}
              </div>
                ...
                {project.links.length === 0 ? (
                  ...
                  project.links.map((link) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-medium text-[--color-text-primary] truncate">{link.label}</p>
          ...
          {(["momentum", "scope", "blockers"] as const).map((tab) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex-1 flex items-center justify-center gap-1.5 py-2.5 text-[10px] font-medium uppercase tracking-wide transition-colors",
                  rightTab === tab
              ...
              >
                ...
                {tab}
                {counts[tab] !== null && (
                  <span className="text-[9px] tabular-nums opacity-60">{counts[tab]}</span>
                )}
              </button>
            );
          })}
          ...
          <div className="p-3">
            ...
            {rightTab === "scope" && (
              <ScopePanel
                ...
                onAdd={onAddScopeItem}
                onMove={onMoveScopeItem}
                onRemove={onRemoveScopeItem}
              />
            )}
...
function BlockerPanel({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {active.length === 0 && (
        <div className="flex flex-col items-center py-6 rounded-lg border border-dashed border-[--color-border]">
          <Shield className="h-6 w-6 text-[--color-success] opacity-40 mb-2" />
          <p className="text-[11px] text-[--color-text-muted]">No active blockers</p>
        </div>
      )}
      ...
      {active.length > 0 && (
        <div className="flex flex-col gap-1.5">
          {active.map((b) => (
            <div
              key={b.id}
              className="flex items-start gap-2 rounded-lg border border-[--color-error]/20 bg-[--color-error]/5 px-3 py-2.5"
            >
              <AlertCircle className="h-3.5 w-3.5 text-[--color-error] shrink-0 mt-0.5" />
              <div className="flex-1 min-w-0">
              ...
              <button
                ...
                className="text-[10px] text-[--color-text-muted] hover:text-[--color-success] transition-colors shrink-0 mt-0.5"
                title="Mark resolved"
              >
                ✓
              </button>
      ...
      {resolved.length > 0 && (
        <div className="flex flex-col gap-1 opacity-50">
          <p className="text-[10px] text-[--color-text-muted] uppercase tracking-wide px-1">
            Resolved ({resolved.length})
          </p>
          {resolved.slice(0, 3).map((b) => (
            <div key={b.id} className="flex items-center gap-2 px-3 py-1.5 rounded border border-[--color-border-subtle]">
              <CheckSquare className="h-3 w-3 text-[--color-success] shrink-0" />
              <span className="text-[11px] text-[--color-text-muted] line-through truncate">{b.title}</span>
            </div>
          ))}
        </div>
      )}
        ...
        return (
          <div key={status}>
            <div className={cn("flex items-center gap-2 mb-1.5", config.color)}>
              {config.icon}
              <span className="text-[11px] font-medium uppercase tracking-wider">{config.label}</span>
...
function TaskRow({ task }: { task: TaskItem }) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 border-b border-[--color-border-subtle] last:border-0 hover:bg-[--color-card-hover] transition-colors group">
      ...
      <span className={cn("flex-1 text-[13px] truncate", isDone ? "text-[--color-text-muted] line-through" : "text-[--color-text-primary]")}>
        {task.title}
      </span>
      <div className="flex items-center gap-2 shrink-0">
        {task.priority !== "MEDIUM" && (
          <span className={cn("text-[11px] font-medium", priorityCfg.color)}>
            <Flag className="h-3 w-3 inline mr-0.5" />{priorityCfg.label}
          </span>
        )}
        {task.dueDate && (
        ...
        <div>
          ...
          {milestone.targetDate && (
            <p className="text-[11px] text-[--color-text-muted] mt-0.5 flex items-center gap-1">
              <Calendar className="h-3 w-3" />{formatDate(milestone.targetDate)}
            </p>
          )}
        </div>
      ...
      {milestone.taskCount > 0 && (
        <div className="flex flex-col gap-1">
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-[--color-text-muted]">{milestone.taskDone}/{milestone.taskCount} tasks</span>
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      {action && (
        <Button size="sm" variant="secondary" onClick={action.onClick} className="mt-3 gap-1.5 text-[12px] h-7">
